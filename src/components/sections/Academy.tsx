...
//               onClick={() => scrollTo(i)}
//               className={`h-1 rounded-full transition-all duration-500 ${active === i ? "w-8 bg-primary" : "w-3 bg-border hover:bg-primary/50"
//                 }`}
//             />
//           ))}
//         </div>
          ...
          >
            {courses.map((c, i) => (
              <motion.article
                ...
                className="glass lux-hover flex w-full shrink-0 snap-start flex-col rounded-sm p-6 will-change-transform md:w-[calc((100%_-_2.5rem)/3)]"
              >
                <h3 className="text-xl">{c.name}</h3>
                <div className="mt-3 space-y-1 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
                  <p className="flex items-center gap-2">
                  ...
                  onClick={(e) => {
                    ...
                    window.dispatchEvent(
                      new CustomEvent("select-enquiry-mode", {
                        detail: { mode: "class", course: c.name },
                      })
                    );
                    document.getElementById("book")?.scrollIntoView({ behavior: "smooth" });
                  }}
