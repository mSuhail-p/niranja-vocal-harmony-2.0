...
//         <SectionHeading eyebrow="Awards & media" title="Recognition, press and appearances." />
  ...
  return (
    <section id="awards" className="bg-royal py-14 sm:py-20">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-10 grid gap-px overflow-hidden rounded-sm border border-border bg-border grid-cols-1 sm:grid-cols-2">
          {awards.map((a, i) => (
            <Reveal key={a.title} delay={(i % 2) * 0.07} className="bg-background">
              <div className="h-full p-8">
                <Award className="size-5 text-primary" />
                <h3 className="mt-5 text-xl leading-snug">{a.title}</h3>
                <p className="mt-2 text-xs uppercase tracking-[0.2em] text-muted-foreground">
